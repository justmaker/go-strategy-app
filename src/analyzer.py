...
class CacheMissError(Exception):
    """Raised when cache-only mode is enabled and no cached result exists."""
...
class GoAnalyzer:
    ...
    Usage:
        ...
        # Cache-only mode (no GPU required)
        analyzer = GoAnalyzer(cache_only=True)
        try:
            result = analyzer.analyze(board_size=19, moves=["B Q16"])
        except CacheMissError:
            print("Position not in cache")
    """
    ...
    def __init__(
        ...
        Args:
            ...
            cache_only: If True, only return cached results (no KataGo needed).
                       Raises CacheMissError when position is not in cache.
        """
        ...
        # Only initialize KataGo if not in cache-only mode
        if not cache_only:
    ...
    def start(self) -> None:
        ...
        In cache-only mode, this is a no-op.
        """
        if self.cache_only:
            return
        if not self._started and self.katago is not None:
            self.katago.start()
            self._started = True
    ...
    def shutdown(self) -> None:
        ...
        Safe to call multiple times. In cache-only mode, this is a no-op.
        """
        if self.cache_only:
            return
        if self._started and self.katago is not None:
    ...
    def analyze(
        ...
        if not force_refresh:
            # In cache-only mode, accept any cached result (don't require exact visits)
            cache_visits = None if self.cache_only else current_visits
            cached = self.cache.get(
                canonical_hash, komi=board.komi, required_visits=cache_visits
            )
        ...
        # Cache miss - check if cache-only mode
        if self.cache_only:
            raise CacheMissError(
                f"Position not in cache (hash={canonical_hash[:16]}..., "
                f"size={board_size}, komi={board.komi})"
            )
        ...
        if self.katago is None:
            raise CacheMissError("KataGo not available in cache-only mode")
    ...
    def _expand_symmetries(self, result: AnalysisResult, board: BoardState) -> None:
        ...
        for canonical, data in groups.items():
            ...
            max_visits = max(
                m.visits for m in data["moves"]
            )  # Visits we can keep max or average, max is probably safer to show it's "known"
    ...
    def _add_empty_board_candidates(
        self, result: AnalysisResult, board: BoardState
    ) -> None:
        """
    ...
    def analyze_board(
        ...
        # Cache miss - check if cache-only mode
        if self.cache_only or self.katago is None:
            raise CacheMissError(
                f"Position not in cache (hash={canonical_hash[:16]}..., "
                f"size={board.size}, komi={board.komi})"
            )
        ...
        # Run analysis with timing
        start_time = time.time()
        top_moves, _ = self.katago.analyze(
            next_player=board.next_player,
            visits=current_visits,
            top_n=top_n,
        )
    ...
    def is_running(self) -> bool:
        """Check if KataGo is running. Always False in cache-only mode."""
        if self.cache_only or self.katago is None:
            return False
