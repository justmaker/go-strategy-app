...
log_file = log_dir / f"opening_book_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def main():
    ...
    logger.info("=" * 60)
    logger.info(f"{args.board_size}x{args.board_size} Opening Book Generation")
    logger.info("=" * 60)
    ...
    logger.info(f"Start time:  {args.start_at or 'Immediately'}")
    logger.info("=" * 60)
    ...
    logger.info("Starting opening book generation...")
    print()
    ...
    with GoAnalyzer() as analyzer:
        ...
        try:
            ...
            logger.info("Summary")
            logger.info("=" * 60)
            logger.info(f"Unique positions processed: {nodes_count}")
            logger.info(f"Cache hits:                 {cache_hits}")
            logger.info(f"Cache misses (new):         {cache_misses}")
            logger.info(f"Total cache entries:        {analyzer.cache.count()}")
            logger.info("=" * 60)
            ...
            if 'queue' in locals() and not queue and nodes_count > 0:
                logger.info("Recording completion in metadata...")
                try:
                    query = """
                    ...
                    logger.info("Successfully recorded opening book completion.")
                except Exception as e:
