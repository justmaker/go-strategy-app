...
class KataGoService {
  ...
  Future<bool> start() async {
    ...
    try {
      ...
      // Start the engine
      final success = await _methodChannel.invokeMethod<bool>('startEngine') ?? false;
      ...
      _errorCallback?.invoke('Failed to start engine: $e');
      return false;
    }
  }
  ...
  Future<String?> analyze({
    ...
    if (!isRunning) {
      onError?.invoke('Engine not running');
      return null;
    }
    ...
    try {
      ...
      onError?.invoke('Analysis failed: $e');
      return null;
    }
  }
  ...
  Future<bool> checkEngineStatus() async {
    try {
      final running = await _methodChannel.invokeMethod<bool>('isEngineRunning') ?? false;
  ...
  void _setupEventListener() {
    ...
    _eventSubscription = _eventChannel.receiveBroadcastStream().listen(
        ...
        if (event is Map) {
          ...
          switch (type) {
            case 'analysis':
              ...
              break;
            case 'error':
              _errorCallback?.invoke(event['message'] as String? ?? 'Unknown error');
              break;
          }
        }
      },
      onError: (error) {
        ...
        _errorCallback?.invoke('Stream error: $error');
      },
    );
  }
  ...
  void _handleAnalysisEvent(String? jsonData) {
    ...
    try {
      ...
      if (data.containsKey('error')) {
        _errorCallback?.invoke(data['error'] as String);
        _currentQueryId = null;
        return;
      }
