...
struct AnalyzeRequest {
  ...
  // If it ever gets to STATUS_POPPED or later, then the analysis thread is reponsible for writing the result, else the
  // api thread is
...
int MainCmds::analysis(const vector<string>& args, std::istream& in, std::ostream& out) {
  Board::initHash();
  ...
  try {
    ...
    TCLAP::ValueArg<int> numAnalysisThreadsArg(
      "",
      "analysis-threads",
      "Analyze up to this many positions in parallel. Equivalent to numAnalysisThreads in the config.",
      false,
      0,
      "THREADS");
    TCLAP::SwitchArg quitWithoutWaitingArg(
      "", "quit-without-waiting", "When stdin is closed, quit quickly without waiting for queued tasks");
  ...
  if(cfg.contains("numAnalysisThreads") && numAnalysisThreadsCmdlineSpecified)
    throw StringError(
      "When specifying numAnalysisThreads in the config (" + cfg.getFileName() +
      "), it is redundant and disallowed to also specify it via -analysis-threads");

  const int numAnalysisThreads =
    numAnalysisThreadsCmdlineSpecified ? numAnalysisThreadsCmdline : cfg.getInt("numAnalysisThreads", 1, 16384);
  ...
  auto loadParams = [&humanModelFile](
                      ConfigParser& config, SearchParams& params, Player& perspective, Player defaultPerspective) {
...
#ifndef USE_EIGEN_BACKEND
  {
    ...
    if(nnMaxBatchSizeTotal * 1.5 <= numThreadsTotal) {
      logger.write(Global::strprintf(
        ...
        numThreadsTotal));
      logger.write(
        "The number of simultaneous threads that might query the GPU could be larger than the batch size that the GPU "
        "will handle at once.");
  ...
  if(humanModelFile != "" && !cfg.contains("humanSLProfile") && humanEval->requiresSGFMetadata()) {
    logger.write(
      "Warning: Provided -human-model but humanSLProfile is not yet set. The human SL model will only be used on "
      "queries that provide humanSLProfile in overrideSettings.");
    if(!logger.isLoggingToStderr())
      cerr << "Warning: Provided -human-model but humanSLProfile is not yet set. The human SL model will only be used "
              "on queries that provide humanSLProfile in overrideSettings."
           << endl;
  }
  ...
  auto reportErrorForId = [&pushToWrite, &logger, &logErrorsAndWarnings](
                            const string& id, const string& field, const string& s) {
    json ret;
    ret["id"] = id;
    ret["field"] = field;
    ret["error"] = s;
  ...
  auto reportWarningForId = [&pushToWrite, &logger, &logErrorsAndWarnings](
                              const string& id, const string& field, const string& s) {
    json ret;
    ret["id"] = id;
    ret["field"] = field;
  ...
  // Report analysis for which we don't actually have results. This is used when something is user-terminated before
  // being actually analyzed properly. Only used outside of search too
  ...
  auto reportAnalysis = [&preventEncore,
                         &pushToWrite](const AnalyzeRequest* request, const Search* search, bool isDuringSearch) {
    json ret;
    ret["id"] = request->id;
    ret["turnNumber"] = request->turnNumber;
    ...
    bool success = search->getAnalysisJson(
      ...
      request->includeOwnership,
      request->includeOwnershipStdev,
      request->includeMovesOwnership,
      request->includeMovesOwnershipStdev,
  ...
  auto analysisLoop = [&logger,
                       ...
                       &nnEval,
                       &openRequestsMutex,
                       &openRequests](AsyncBot* bot, int threadIdx) {
    while(true) {
      ...
      // Else, the request is live and we marked it as popped
        ...
        bot->setAlwaysIncludeOwnerMap(
          request->includeOwnership || request->includeOwnershipStdev || request->includeMovesOwnership ||
          request->includeMovesOwnershipStdev);
        ...
        if(request->reportDuringSearch) {
          ...
          bot->genMoveSynchronousAnalyze(
            ...
            TimeControls(),
            searchFactor,
            request->reportDuringSearchEvery,
            request->firstReportDuringSearchAfter,
            callback,
            onSearchBegun);
        } else {
...

        {
          ...
          // If the search didn't have any root or root neural net output, it must have been interrupted and we must be
          // quitting imminently
          if(!analysisWritten) {
            // If the reason we stopped was because we noticed a terminate, then we will write out a dummy response even
            // if we didn't have enough info to generate a real one, to fulfill a promise in the API docs that we always
            // write something.
            ...
            // Otherwise, this case is only possible if we're just shutting down
              ...
              logger.write(
                "Note: Search quitting due to no visits - this is normal and possible when shutting down but a bug "
  ...
  auto terminateRequest = [&bots, &reportNoAnalysis](AnalyzeRequest* request) {
    ...
    // A thread popped it. That thread will notice that it's terminated once it tries to put its thread idx in, so we
    ...
    // A thread started searching it and put its thread idx in
      ...
      assert(prevStatus >= 0);
      // We've already set the above status to terminated so when the thread terminates due to our killing it below, it
      // will see this. Or else the thread has already done so, in which case it's already properly written a result,
      // also fine.
    ...
    while(getline(in, line)) {
      ...
      if(input.find("action") != input.end() && input["action"].is_string()) {
        ...
        if(action == "query_version") {
          ...
          if(!terminateIdFound) {
            reportErrorForId(
              rbase.id, "terminateId", "Requests for a terminate action must have a string \"terminateId\" field");
            continue;
          }
          ...
          if(input.find("turnNumbers") != input.end()) {
            try {
              ...
              reportErrorForId(
                rbase.id, "turnNumbers", "If provided, must be an array of integers indicating turns to terminate");
              continue;
            }
          }

          {
            ...
            for(auto it = openRequests.begin(); it != openRequests.end(); ++it) {
              ...
              if(
                request->id == terminateId &&
                (!hasTurnNumbers || (turnNumbersSet.find(request->turnNumber) != turnNumbersSet.end())))
      ...
      auto parseInteger =
        [&rbase, &reportErrorForId](
          const json& dict, const char* field, int64_t& buf, int64_t min, int64_t max, const char* errorMessage) {
          try {
      ...
      auto parseDouble =
        [&rbase, &reportErrorForId](
          const json& dict, const char* field, double& buf, double min, double max, const char* errorMessage) {
          try {
      ...
      auto parseBoolean = [&rbase, &reportErrorForId](
                            const json& dict, const char* field, bool& buf, const char* errorMessage) {
        try {
      ...
      int boardYSize;
      {
        int64_t xBuf;
        int64_t yBuf;
        static const string boardSizeError =
          string("Must provide an integer from 2 to ") + Global::intToString(Board::MAX_LEN);
      ...
      auto parseBoardLocs = [boardXSize, boardYSize, &rbase, &reportErrorForId](
                              const json& dict, const char* field, vector<Loc>& buf, bool allowPass) {
        buf.clear();
        ...
        for(auto& elt: dict[field]) {
          ...
          Loc loc;
          if(
            !Location::tryOfString(s, boardXSize, boardYSize, loc) || (!allowPass && loc == Board::PASS_LOC) ||
            (loc == Board::NULL_LOC)) {
      ...
      auto parseBoardMoves = [boardXSize, boardYSize, &rbase, &reportErrorForId](
                               const json& dict, const char* field, vector<Move>& buf, bool allowPass) {
        buf.clear();
        if(!dict[field].is_array()) {
          reportErrorForId(
            rbase.id, field, "Must be an array of pairs of the form: [\"b\" or \"w\", GTP board vertex]");
          return false;
        }
        for(auto& elt: dict[field]) {
          ...
          Loc loc;
          if(
            !Location::tryOfString(s1, boardXSize, boardYSize, loc) || (!allowPass && loc == Board::PASS_LOC) ||
            (loc == Board::NULL_LOC)) {
      ...
      if(input.find("rules") != input.end()) {
        if(input["rules"].is_string()) {
          ...
          reportErrorForId(
            rbase.id,
            "rules",
            "Must specify rules string, such as \"chinese\" or \"tromp-taylor\", or a JSON object with detailed rules "
            "parameters.");
          continue;
        }
      } else {
      ...
      if(input.find("overrideSettings") != input.end()) {
        ...
        for(auto it = settings.begin(); it != settings.end(); ++it) {
          overrideSettings[it.key()] =
            it.value().is_string() ? it.value().get<string>() : it.value().dump();  // always convert to string
        }
        ...
        if(!overrideSettings.empty()) {
          try {
            ...
            if(unusedKeys.size() > 0) {
              reportWarningForId(
                rbase.id, "overrideSettings", string("Unknown config params: ") + Global::concat(unusedKeys, ","));
            }
            ostringstream out;
      ...
      if(input.find("maxVisits") != input.end()) {
        bool suc = parseInteger(
          input, "maxVisits", rbase.params.maxVisits, 1, (int64_t)1 << 50, "Must be an integer from 1 to 2^50");
        if(!suc)
          continue;
      }
      ...
      if(input.find("rootFpuReductionMax") != input.end()) {
        bool suc = parseDouble(
          input, "rootFpuReductionMax", rbase.params.rootFpuReductionMax, 0.0, 2.0, "Must be a number from 0.0 to 2.0");
        if(!suc)
          continue;
      }
      if(input.find("rootPolicyTemperature") != input.end()) {
        bool suc = parseDouble(
          input,
          "rootPolicyTemperature",
          rbase.params.rootPolicyTemperature,
          0.01,
          100.0,
      ...
      if(input.find("includeMovesOwnershipStdev") != input.end()) {
        bool suc =
          parseBoolean(input, "includeMovesOwnershipStdev", rbase.includeMovesOwnershipStdev, "Must be a boolean");
        if(!suc)
          continue;
      }
      ...
      if(input.find("firstReportDuringSearchAfter") != input.end()) {
        bool suc = parseDouble(
          input,
          "firstReportDuringSearchAfter",
          rbase.firstReportDuringSearchAfter,
          0.001,
          1000000.0,
          "Must be number of seconds from 0.001 to 1000000.0");
        if(!suc)
      ...
      if(hasAllowMoves || hasAvoidMoves) {
        ...
        if(!avoidParamsList.is_array()) {
          reportErrorForId(
            rbase.id, field, string("Must be a list of dicts with subfields 'player', 'moves', 'untilDepth'"));
          continue;
        }
        ...
        for(size_t i = 0; i < avoidParamsList.size(); i++) {
          ...
          if(
            avoidParams.find("moves") == avoidParams.end() || avoidParams.find("untilDepth") == avoidParams.end() ||
          ...
          vector<int>& avoidMoveUntilByLoc =
            avoidPla == P_BLACK ? rbase.avoidMoveUntilByLocBlack : rbase.avoidMoveUntilByLocWhite;
        ...
        for(auto it = input.begin(); it != input.end(); ++it) {
          if(expectedKeys.find(it.key()) == expectedKeys.end())
            reportWarningForId(
              rbase.id,
              it.key(),
              "Unexpected or unused field, do you have a typo? (set warnUnusedFields=false in the config to disable "
              "this warning)");
        }
      }
      ...
      for(int turnNumber = 0; turnNumber <= moveHistory.size(); turnNumber++) {
        ...
        if(!suc) {
          reportErrorForId(
            rbase.id,
            "moves",
            "Illegal move " + Global::intToString(turnNumber) + ": " + Location::toString(moveLoc, board));
  ...
  if(quitWithoutWaiting) {
    // Making this readOnly will halt futher output that isn't already queued and signal the write loop thread to
    // terminate.
