...
class TestParseKataAnalyzeLine:
    ...
    def test_parse_pass(self, katago):
        ...
        assert moves[0].visits == 100
        assert moves[0].winrate == 0.1
        assert moves[0].score_lead == -5.0


class TestParseAnalyzeOutput:
    """Tests for _parse_analyze_output method (multi-line format)."""
    ...
    def test_parse_multiline_output(self, katago):
        """Test parsing multi-line output (typical GTP response)."""
        output = (
            "info move Q3 visits 45 winrate 0.523 scoreLead 0.312\n"
            "info move R4 visits 38 winrate 0.518 scoreLead 0.287\n"
            "info move D4 visits 10 winrate 0.4 scoreLead 0.1"
        )
        moves = katago._parse_analyze_output(output, top_n=3)

        assert len(moves) == 3
        assert moves[0].move == "Q3"
        assert moves[1].move == "R4"
        assert moves[2].move == "D4"

    def test_parse_multiline_with_garbage(self, katago):
        """Test parsing multi-line output with some garbage lines."""
        output = (
            "info move Q3 visits 45 winrate 0.523 scoreLead 0.312\n"
    ...
    def test_parse_multiline_scientific(self, katago):
        """Test parsing multi-line output with scientific notation."""
        output = "info move Q3 visits 100 winrate 1.5e-2 scoreLead -1.2e1"
        moves = katago._parse_analyze_output(output, top_n=1)
